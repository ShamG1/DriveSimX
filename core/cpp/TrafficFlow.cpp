...
static inline void enforce_npc_route_snap_tf(Car& npc) {
    if (!npc.alive) return;
    if (npc.path.size() < 2) return;
    const float x = npc.state.x;
    const float y = npc.state.y;
    int best_seg = -1;
    float best_u = 0.0f;
    float best_d2 = std::numeric_limits<float>::infinity();
    for (int i = 0; i + 1 < (int)npc.path.size(); ++i) {
        const float ax = npc.path[i].first;
        const float ay = npc.path[i].second;
        const float bx = npc.path[i + 1].first;
        const float by = npc.path[i + 1].second;
        const float sx = bx - ax;
        const float sy = by - ay;
        const float ss = sx * sx + sy * sy;
        if (ss <= 1e-6f) continue;
        float u = ((x - ax) * sx + (y - ay) * sy) / ss;
        u = clampf_tf(u, 0.0f, 1.0f);
        const float px = ax + u * sx;
        const float py = ay + u * sy;
        const float dx = x - px;
        const float dy = y - py;
    ...
    if (best_seg < 0) return;
    const float ax = npc.path[best_seg].first;
    const float ay = npc.path[best_seg].second;
    const float bx = npc.path[best_seg + 1].first;
    const float by = npc.path[best_seg + 1].second;
    float tx = bx - ax;
    float ty = by - ay;
    const float tlen = std::hypot(tx, ty);
    if (tlen <= 1e-6f) return;
    tx /= tlen;
    ty /= tlen;
    const float pix = ax + best_u * (bx - ax);
    const float piy = ay + best_u * (by - ay);
    const float rx = x - pix;
    const float ry = y - piy;
    // Decompose position error into longitudinal (et) and lateral (en).
    const float et = rx * tx + ry * ty;
    // Strict projection: clear lateral error (en -> 0).
    npc.state.x = pix + et * tx;
    npc.state.y = piy + et * ty;
    // Clamp heading around route tangent to keep visual lane adherence.
    const float route_heading = std::atan2(-ty, tx);
    const float heading_err = wrap_angle_rad_tf(npc.state.heading - route_heading);
    const float max_heading_err = 10.0f * PI_F_TF / 180.0f;
    const float clamped_err = clampf_tf(heading_err, -max_heading_err, max_heading_err);
    npc.state.heading = wrap_angle_rad_tf(route_heading + clamped_err);
}
static inline std::pair<float, float> plan_npc_action_tf(const Car& npc, const std::vector<const Car*>& all_vehicles) {
    ...
    const float v0 = PHYSICS_MAX_SPEED * 0.1f;
